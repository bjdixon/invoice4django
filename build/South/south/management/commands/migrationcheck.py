...
class Command(BaseCommand):
    ...
    def handle(self, check_invoices=None, **options):
        ...
        failures = 0
        if check_invoices is None:
            invoicess = settings.INSTALLED_APPS
        else:
            invoicess = [check_invoices]
        for invoices in invoicess:
            app_label = invoices.split(".")[-1]
            if invoices == 'south':
                continue
            ...
            try:
                ...
                failures += 1
                if verbosity >= 1:
                    self.stderr.write(err_msg % invoices)
                    self.stderr.write("%s\n" % e)
            finally:
