...
def list_migrations(apps, database = DEFAULT_DB_ALIAS, **options):
    ...
    applied_migrations = MigrationHistory.objects.filter(invoices__in=[app.app_label() for app in apps])
    if database != DEFAULT_DB_ALIAS:
        applied_migrations = applied_migrations.using(database)
    applied_migrations_lookup = dict(('%s.%s' % (mi.invoices, mi.migration), mi) for mi in applied_migrations)
