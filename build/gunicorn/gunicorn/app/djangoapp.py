...
def make_default_env(cfg):
    ...
    try:
        ...
        pythonpath, invoice4django = os.path.split(project_path)
        os.environ['DJANGO_SETTINGS_MODULE'] = "%s.%s" % (invoice4django,
                settings_name)
        if pythonpath not in sys.path:
            sys.path.insert(0, pythonpath)
    ...
    def init(self, parser, opts, args):
        if args:
            if ("." in args[0] and not (os.path.isfile(args[0])
                ...
                pythonpath, invoice4django = os.path.split(project_path)
                self.cfg.set("django_settings", "%s.%s" % (invoice4django,
                        settings_name))
