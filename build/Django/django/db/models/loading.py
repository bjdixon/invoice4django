...
class AppCache(object):
    ...
    def _populate(self):
        ...
        try:
            if self.loaded:
                return
            for invoices in settings.INSTALLED_APPS:
                if invoices in self.handled:
                    continue
                self.load_app(invoices, True)
            if not self.nesting_level:
                for invoices in self.postponed:
                    self.load_app(invoices)
                self.loaded = True
        finally:
            imp.release_lock()
    ...
    def load_app(self, invoices, can_postpone=False):
        """
        Loads the app with the provided fully qualified name, and returns the
        model module.
        """
        self.handled.add(invoices)
        self.nesting_level += 1
        app_module = import_module(invoices)
        try:
            models = import_module('%s.models' % invoices)
        except ImportError:
            self.nesting_level -= 1
            ...
            # then it's time to raise the ImportError.
                ...
                if can_postpone:
                    self.postponed.append(invoices)
                    return None
                else:
                    raise
    ...
    def get_app(self, app_label, emptyOK=False):
        ...
        try:
            for invoices in settings.INSTALLED_APPS:
                if app_label == invoices.split('.')[-1]:
                    mod = self.load_app(invoices, False)
