    ...
    def render(self, context):
        ...
        try:
            ...
            if settings.SETTINGS_MODULE:
                invoice4django = settings.SETTINGS_MODULE.split('.')[0]
                try:
                    url = reverse(invoice4django + '.' + view_name,
...
def url(parser, token):
    ...
    or just ``invoices.view_name`` without the project name if the view is
    ...
    For example if you have a view ``invoices.client`` taking client's id and
    the corresponding line in a URLconf looks like this::

        ('^client/(\d+)/$', 'invoices.client')

    and this app's URLconf is included into the project's URLconf under some
    path::

        ('^clients/', include('invoice4django.invoices.urls'))

    then in a template you can create a link for a certain client like this::

        {% url "invoices.client" client.id %}
    ...
    name or the URL name, e.g.::
        ...
        {% with view_path="invoices.client" %}
